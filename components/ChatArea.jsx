...
import { addDoc, arrayRemove, arrayUnion, collection, doc, getDoc, serverTimestamp, setDoc, Timestamp, updateDoc } from "firebase/firestore";
...
export function ChatArea({ messages, requestedUserData, loggedInUserData }) {
    ...
    const handleSendText = () => {
        const loggedInUserChatDocument = doc(db, `users/${loggedInUserData.uid}/chats/${requestedUserData.uid}`);
        const requestedUserChatDocument = doc(db, `users/${requestedUserData.uid}/chats/${loggedInUserData.uid}`);
    ...
    return (
        <Grid container direction="column" alignItems="center" justifyContent="space-between" padding="1rem" sx={{ width: "100%", height: "100%" }}>
